...
//! `std::io` API and the `midly_smf::io` API.
//! Besides, `write` methods that work with `midly_smf::io::Write` types usually provide a `write_std`
...
/// A `Write` trait available even in `no_std` environments, and with per-writer errors.
pub trait Write {
    /// The error type specific to this writer.
    type Error;
    /// Encodes whether the writer is seekable within the type system.
    ///
    ...
    ///
    /// Used when encoding MIDI files with invalid data.
...
///
/// Not all of the `Seek` functionality is required: writing MIDI files only requires writing a
/// small chunk of data at an arbitrary position every once in a while.
...
/// Equivalent to the `!` "never" type.
#[derive(Copy, Clone, Debug)]
enum Never {}

/// The type used for the [`Write::Seekable`] associated type on
...
impl<W> NotSeekable<W> {
    /// The `NotSeekable` type is not constructable, and as such can be casted to the `!` type.
    #[inline]
...
impl<'a> Write for Cursor<'a> {
    ...
    #[inline]
    fn write(&mut self, buf: &[u8]) -> WriteResult<Self> {
        //Cannot overflow because `self.cur <= self.buf.len()` is always true.
...
pub enum CursorError {
    ...
    OutOfSpace,
    /// The SMF that was being encoded was invalid.
...
/// A writer that counts the amount of bytes written to it, but otherwise ignores the actual bytes
/// written.
